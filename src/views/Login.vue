  ...
  authorization
    ...
    .then(function (result) {
      ...
      if (result && result.access_token) {
        let payload = String(result.access_token.split(".")[1]).replace(
          /_/g,
          "/"
        );
...
<template>
  <div class="wrap-login">
    <v-container id="login-page" class="px-0 pt-0" fluid tag="section">
      <div class="container-wrap">
        <div class="wrap-title">
          <div class="d-flex justify-end py-0 pb-0 my-0 px-5"></div>
          <v-row align="center">
            <v-col class="py-0">
              <img
                class="img-login-logo"
                :src="`${publicPath}/images/logo-bca-bigger.png?t=93111413`"
              />
            </v-col>
          </v-row>

          <v-row class="wrap-title pt-1 my-0 mt-3">
            <v-col cols="12" class="text-1 py-2 my-0 mb-2"
              >CỔNG THÔNG TIN QUỐC GIA
            </v-col>
            <v-col cols="12" class="text-2 py-2"
          ...
          <div>
            <v-form ref="form" v-model="valid" lazy-validation class="">
              ...
              <v-row align="center" class="wrap-btn-login my-3">
                <v-col>
                  <v-btn
                    class="my-0 white--text mr-3 btn-login"
                    style="padding: 0 15px !important"
                    :loading="loading"
                    :disabled="loading"
                    @click="login"
                  >
                    <v-icon size="20" icon="mdi:mdi-login"></v-icon>&nbsp; Đăng
                    nhập
                  </v-btn>
                </v-col>
              </v-row>
        ...
        <div class="wrap-form px-3 py-3" v-if="signed">
          ...
          <v-row align="center" style="margin: 20px 0; text-align: center">
            <v-col cols="12">
              ...
              <v-btn
                class="my-0 white--text mr-3 mt-3 btn-login"
                small
                style="padding: 0 15px !important"
                :loading="loading"
                :disabled="loading"
...
.wrap-login .v-field--variant-filled .v-field__outline::before,
.wrap-login .v-field--variant-underlined .v-field__outline::before,
.wrap-login .v-field--variant-filled .v-field__outline::after,
.wrap-login .v-field--variant-underlined .v-field__outline::after {
  display: none !important;
}
#login-page {
  position: relative;
...
.input-text .v-input__slot {
  box-shadow: 0px 3px 1px -2px rgb(0 0 0 / 8%), 0px 2px 2px 0px rgb(0 0 0 / 0%),
